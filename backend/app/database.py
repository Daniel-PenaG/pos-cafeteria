...
def aplicar_migraciones_sqlite():
    ...
    migraciones = (
        ...
        + migraciones_mesas_pg + migraciones_cierres_modulos_pg
        ...
        + migraciones_mesas_sqlite + migraciones_cierres_modulos_sqlite
    )
    ...
    aplicar_indices_performance()


def aplicar_indices_performance():
    """Índices para filtros/joins frecuentes. Idempotente (IF NOT EXISTS)."""
    dialect = engine.dialect.name
    indices_pg = [
    ...
    indices = indices_pg if dialect == "postgresql" else indices_sqlite
    for sql in indices:
        try:
            with engine.begin() as conn:
                conn.execute(text(sql))
        except Exception as exc:
            print(f"[WARN] Indice no aplicado: {sql[:60]}... -> {exc}")
